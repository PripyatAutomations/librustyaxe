...
static void print_key(TermKey *tk, TermKeyKey *key)
{
   ...
   int m = key->modifiers;
   fprintf( stderr, " mod=%s%s%s+%02x", (m & TERMKEY_KEYMOD_CTRL ? "C" : ""),
      (m & TERMKEY_KEYMOD_ALT ? "A" : ""), (m & TERMKEY_KEYMOD_SHIFT ? "S" : ""),
 ...
 *    it takes char**s and updates them n counts bytes of strcamel, not str
 */
...
TermKeyResult termkey_waitkey(TermKey *tk, TermKeyKey *key)
{
   ...
   while (1) {
      ...
      switch (ret) {
         ...
         {
            if (tk->is_closed) {
               // We're closed now. Never going to get more bytes so just go
               // with
               // what we have
 ...
 */
size_t termkey_snprint_key(TermKey *tk, char *buffer, size_t len, TermKeyKey *key,
                           TermKeyFormat format)
{
...
size_t termkey_strfkey(TermKey *tk, char *buffer, size_t len, TermKeyKey *key, TermKeyFormat format)
{
   ...
   switch (key->type) {
      case TERMKEY_TYPE_UNICODE: {
         if (!key->utf8[0]) {
            // In case of user-supplied key structures
            fill_utf8(key);
         }
      ...
      case TERMKEY_TYPE_MOUSE:
      {
         ...
         l = snprintf(buffer + pos, len - pos, "Mouse%s(%d)", evnames[ev], button);
...
const char *termkey_strpkey(TermKey *tk, const char *str, TermKeyKey *key, TermKeyFormat format)
{
   ...
   if ( ( endstr = termkey_lookup_keyname_format(tk, str, &key->code.sym, format) ) ) {
      ...
      if ( (format & TERMKEY_FORMAT_MOUSE_POS) && sscanf(str, " @ (%u,%u)%zn", &col, &line,
         &snbytes) == 2 ) {
         str += snbytes;
      }
   ...
   else if (parse_utf8( (unsigned const char *)str, strlen(str), &key->code.codepoint,
      &nbytes ) == TERMKEY_RES_KEY) {
      key->type = TERMKEY_TYPE_UNICODE;
      fill_utf8(key);
      str += nbytes;
   }else {
