...
/*
 * Here we deal with a few different ways of accessing flash depending on how
 * it's connected.
 *
...
bool eeprom_corrupted;                   // EEPROM is corrupted; prompt before
                                         // write out
time_t eeprom_saved;                     // When was EEPROM last written out?
time_t eeprom_changed;                   // When was a setting last changed that
                                         // needs written to EEPROM?
...
uint32_t eeprom_load_config(void) {
   ...
   if (eeprom_validate_checksum() != 0) {
      ...
      // XXX: Here we should load some default network and authentication
      // settings...
      return -1;
   }
   ...
   for (uint32_t i = 0 ; i < cfg_rows ; i++) {
      ...
      switch (eeprom_layout[i].type) {
         ...
         case EE_CHAN_SLOT: {
            // XXX: we need to parse channel data - needs buildconf to add
            // structs
            // to eeprom_types.h
            chan_slots_loaded++;
...
//   EE_MODE,                     /* Operating mode (modulation) */
         default: {
            Log(LOG_CRIT, "eeprom", "unhandled type %d while parsing eeprom layout",
               eeprom_layout[i].type);
...
#if     defined(NOISY_EEPROM)
      Log(LOG_DEBUG, "eeprom", "enumerate: %s <%d> type %d is %d bytes @ %d |%s|",
         eeprom_layout[i].key, i, eeprom_layout[i].type, eeprom_layout[i].size,
...
#if     defined(NOISY_EEPROM)
   Log(LOG_DEBUG, "eeprom", "get_int: <%i> has offset %d @ %x |%d|", idx, eeprom_layout[idx].offset,
      myaddr, value);
#endif
...
#if     defined(NOISY_EEPROM)
   Log(LOG_DEBUG, "eeprom", "get_float: <%i> has offset %d @ %x |%f|", idx,
      eeprom_layout[idx].offset, myaddr, value);
#endif
...
#if     defined(NOISY_EEPROM)
   Log(LOG_DEBUG, "eeprom", "get_str: <%i> has offset %d @ %x |%s|", idx, eeprom_layout[idx].offset,
      myaddr, buf);
#endif
...
//      errno = ERANGE;
   ...
   u_int8_t *myaddr = eeprom_mmap + eeprom_layout[idx].offset;
#if     defined(NOISY_EEPROM)
   Log( LOG_DEBUG, "eeprom", "get_bool: <%i> has offset %d @ %x |%d=%s|", idx,
...
void show_pin_info(void) {
   ...
   if (show) {
      ...
      Log(LOG_INFO, "eeprom",
         "*** Master PIN: %s, Factory Reset PIN: %s (set pin.show to 0 to hide!) ***", master_pin,
         reset_pin);
   }
}
