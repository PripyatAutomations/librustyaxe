...
/**
 *  This hash function has been taken from an Article in Dr Dobbs Journal. There
 * are probably better ones out there but this one does the job.
 */
...
/** Lookup an element in a dict This implementation copied almost verbatim from
 * the Python dictionary object, without the Pythonisms.
 */
...
/** Add an item to a dictionary without copying key/val Used by dict_resize()
 * only.
 */
...
static int dict_resize(dict *d) {
   ...
   newsize = d->size;
   /*
    * Re-sizing factor depends on the current dict size. Small dicts will expand
